...
#
    ...
    def write_track_id3(self, track_fp, album_artwork: bytes = None):
        """ Write track meta & artwork """

        # Ensure file is written fully
        track_fp.flush()

        # Reopen by filename (needed for mutagen)
        filename = track_fp.name
        ...
        if self.track_no:
            audio.tags.add(TRCK(encoding=3, text=str(self.track_no)))
        ...
        audio.save(v2_version=3)  # save ID3v2.3 for max compatibility
        self.ready = True
        track_fp.seek(0)
        return track_fp
